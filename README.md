...
2. **A PO token provider**, because cookies alone still aren't enough — YouTube's newer "SABR" system additionally hides most formats behind a proof-of-origin token normally generated by running real browser JavaScript. [nixpacks.toml](nixpacks.toml) adds Python to the Railway build so a proper (non-frozen) `yt-dlp` plus the [bgutil-ytdlp-pot-provider](https://github.com/Brainicism/bgutil-ytdlp-pot-provider) plugin can be pip-installed — yt-dlp's standalone binary builds cannot load this plugin at all (confirmed directly: `--plugin-dirs` is silently ignored by frozen builds). [src/music/potProvider.js](src/music/potProvider.js) downloads and runs the actual token-generating server the plugin talks to.
