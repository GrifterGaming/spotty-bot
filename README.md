...
2. **A PO token provider**, because cookies alone still aren't enough — YouTube's newer "SABR" system additionally hides most formats behind a proof-of-origin token normally generated by running real browser JavaScript. Railway's builder (Railpack) doesn't have Python by default; [railpack.json](railpack.json) adds `python3`/`python3-pip` to the deploy image. From there, [src/music/YtDlpSearchPlugin.js](src/music/YtDlpSearchPlugin.js) pip-installs a real (non-frozen) `yt-dlp` plus the [bgutil-ytdlp-pot-provider](https://github.com/Brainicism/bgutil-ytdlp-pot-provider) plugin itself on first startup — yt-dlp's standalone binary builds cannot load this plugin at all (confirmed directly: `--plugin-dirs` is silently ignored by frozen builds), so a genuine Python install is required. [src/music/potProvider.js](src/music/potProvider.js) downloads and runs the actual token-generating server the plugin talks to.
