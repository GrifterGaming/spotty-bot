...
2. **A PO token provider attempt (partially built, not fully working)**: YouTube's newer "SABR" system additionally hides most formats behind a proof-of-origin token normally generated by running real browser JavaScript, and cookies alone don't get past it. [railpack.json](railpack.json) adds `python3`/`python3-pip` to Railway's deploy image, and [src/music/YtDlpSearchPlugin.js](src/music/YtDlpSearchPlugin.js) pip-installs a real (non-frozen) `yt-dlp` plus the [bgutil-ytdlp-pot-provider](https://github.com/Brainicism/bgutil-ytdlp-pot-provider) plugin on first startup — yt-dlp's standalone binary builds cannot load this plugin at all (confirmed directly: `--plugin-dirs` is silently ignored by frozen builds), so a genuine Python install was required just to make the plugin loadable. [src/music/potProvider.js](src/music/potProvider.js) downloads and runs the actual token-generating server the plugin talks to (a standalone Rust reimplementation, no build step). This infrastructure is left in place and does no harm sitting unused, but forcing yt-dlp to actually invoke the provider (`player_client=web`) caused requests to hang indefinitely in production, which is worse than the current behavior — so that forcing was reverted. Currently the provider is set up but not actively engaged; `formats=missing_pot` (below) is what actually keeps things working.
3. **`formats=missing_pot`** (a yt-dlp extractor-arg, always applied): tells yt-dlp to return formats even when it can't get a PO token for them, rather than erroring out. This is the thing actually keeping playback working today — some individual videos still fail with "Requested format is not available" (SABR-restricted with no fallback format at all), but most succeed.
